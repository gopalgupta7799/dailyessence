...
const getItems = async () => {
  try {
    const { data } = await axios.get("/getgrainsoils");
    return data
  } catch (error) {
    console.log(error)
  }
...
const createItem = async (todo) => {
  try {
    const { data } = await axios.post("/addgrainsoil", todo);
    return data
  } catch (error) {
    console.log(error)
  }
...
function AdminGrainsOils() {
  ...
  const onSubmitHandler = async (e) => {
    ...
    document.getElementById("frmIDg").reset();
  }
  ...
  const removeItem = async (_id) => {
    ...
    if (x) {
      await axios.get('/deletegrainsoil/' + _id)
        .then()
  ...
  return (
    <div className="container">
      ...
      <button type="button" className="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModalg">
        Add New Item
      </button>

      {/* <!-- Modal --> */}
      <div className="modal fade" id="exampleModalg" tabIndex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
