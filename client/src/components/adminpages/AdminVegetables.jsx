...
const createItem = async (todo) => {
  try {
    const { data } = await axios.post("/addvegetable", todo);
    return data
  } catch (error) {
    console.log(error)
  }
...
function AdminVegetables() {
  ...
  const onSubmitHandler = async (e) => {
    ...
    document.getElementById("frmIDv").reset();
  }
  ...
  const removeItem = async (_id) => {
    ...
    if (x) {
      await axios.get('/deletevegetable/' + _id)
        .then()
  ...
  return (
    <div className="container">
      ...
      <button type="button" className="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModalv">
        Add New Item
      </button>

      {/* <!-- Modal --> */}
      <div className="modal fade" id="exampleModalv" tabIndex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
